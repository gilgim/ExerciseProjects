...
struct DetailPartFormModel: Model {
    ...
    func readObject(key: String) throws -> DetailPartFormStruct? {
        ...
        //    json 값을 받아옵니다.
        guard let jsonString = realm.object(ofType: DetailPartFormRealm.self, forPrimaryKey: key)?.jsonString else {
            CommonFunction.componentDetailprint()
    ...
    func deleteObject(value: DetailPartFormStruct) throws {
        guard let realm else {
            CommonFunction.componentDetailprint()
            throw ErrorType.RealmDataLookUpError
        }
        try realm.write({
            guard let object = realm.object(ofType: DetailPartFormRealm.self, forPrimaryKey: value.name) else {
                CommonFunction.componentDetailprint()
                throw ErrorType.RealmDataLookUpError
            }
            realm.delete(object)
 ...
 */
struct DetailPartFormStruct: Codable, Equatable {
    /// 세부부위가 할당된 대근육 입니다.
    ...
    /// 구조체의 비교를 위한 함수입니다.
    static func == (lhs: DetailPartFormStruct, rhs: DetailPartFormStruct) -> Bool {
        var value: Bool = false
        if (lhs.name == rhs.name) && (lhs.affiliatedPart == rhs.affiliatedPart) {
            value = true
        }
        return value
    }
